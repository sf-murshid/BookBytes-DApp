...
const Product = ({ item, provider, account, eBook, togglePop }) => {
  const [order, setOrder] = useState(null)
  const [hasBought, setHasBought] = useState(false)

  const fetchDetails = useCallback(async () => {
    const events = await eBook.queryFilter("Buy");
    ...
    const order = await eBook.orders(account, orders[0].args.orderId);
    setOrder(order);
  }, [eBook, account, item.id]);
  ...
  const buyHandler = async () => {
    ...
    // Buy item...
    let transaction = await eBook.connect(signer).buy(item.id, { value: item.cost })
    ...
    <div className="product">
      <div className="product__details">
        ...
        <div className="product__order">
          ...
          <p><small>Ships from</small> BookBytes</p>
          <p><small>Sold by</small> BookBytes</p>
